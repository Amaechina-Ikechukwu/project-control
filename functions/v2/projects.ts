...
const createProject = async (
  ...
  try {
    ...
    const projectsRef = db.ref(`${user}/projects`);

    // Check if the name already exists
    const nameExists = await checkIfProjectNameExists(user, name);
    if (nameExists) {
    ...
    const createdBy = req.headers.get("user-agent") || "Unknown";
 ...
 */
const checkIfProjectNameExists = (
  user: string,
  name: string
): Promise<boolean> => {
  const db = getDatabase();
...
const getPaidStatus = async (
  ...
  try {
    const db = getDatabase();
    const projectRef = db.ref(`${user}/projects/${projectId}/paidStatus`);
 ...
 * @returns A success message or throws an error.
 */
const setPaidStatusFalse = async (
  user: string,
  projectId: string
): Promise<string> => {
  try {
